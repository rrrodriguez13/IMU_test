#!/usr/bin/env python3
"""
plot_raw_angles.py - plot raw per-IMU roll/pitch/yaw from a twoaxis_raw CSV log

    python3 plot_raw_angles.py logs/twoaxis_raw000.csv
    python3 plot_raw_angles.py logs/twoaxis_raw0*.csv --overlay
    python3 plot_raw_angles.py logs/twoaxis_raw000.csv --y imu0_roll_deg,imu1_roll_deg
    python3 plot_raw_angles.py logs/twoaxis_raw000.csv --unwrap
"""
...
# Colors for roll, pitch, yaw - same as imu_collect_accel.py live plot
...
def expand_inputs(inputs):
    # Expand any glob patterns in the file list (e.g. logs/twoaxis_raw0*.csv)
    paths, seen = [], set()
    for item in inputs:
...
def detect_imu_indices(columns):
    # Find which IMU numbers are present in the CSV columns (e.g. imu0, imu1)
    indices = set()
    for col in columns:
...
def maybe_unwrap(data, cols):
    # Unwrap angle columns so they don't jump at the +/-180 degree boundary
    for col in cols:
...
def main():
    ...
    ap.add_argument("--y", default=None, help="comma-separated columns to plot")
    ap.add_argument("--overlay", action="store_true",
                    help="plot all files on the same axes instead of separately")
